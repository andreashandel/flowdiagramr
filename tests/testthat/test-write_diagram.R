...
test_that("write_diagram function works",
          {
            ...
            write_diagram(model_list = model_list, filename = "test_code1.R", always_overwrite = TRUE)
            expect_true(fs::file_exists('./test_code1.R'))
            fs::file_delete('test_code1.R')

            # generate R code from diagram_list
            write_diagram(diagram_list = diagram_list, filename = "test_code2.R", always_overwrite = TRUE)
            expect_true(fs::file_exists('./test_code2.R'))
            fs::file_delete('test_code2.R')

            #' # generate R code from both
            write_diagram(model_list = model_list, diagram_list = diagram_list, filename = "test_code3.R", always_overwrite = TRUE)
            expect_true(fs::file_exists('./test_code3.R'))
