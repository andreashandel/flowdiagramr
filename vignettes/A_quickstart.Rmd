...
We'll finish with one more model, this time doing a more complex one.
...
In the first example below, we use typical math structure to embed interactions among variables in parentheses.
Unfortunately, **`r pkg`** currently cannot parse those interactions (we expect that a future version will), but it does return a (hopefully) informative error message.
