...
You previously learned that to specify a model, you need to set up a list that contains at minimum the elements `varlabels` and `flows`. It is possible to supply a few additional elements, namely the element `varnames` which contains the full names for each variable/compartment and an element `varlocations`, which contains a matrix showing the grid-based arrangement of the compartment boxes. Here is an example for the SIR model you saw previously.
...
Note that we provided the `make_diagram` fuction with an option that specified that we wanted to use the variable names to label the boxes instead of the labels/abbreviations. There are a number of options you can set when calling `make_diagram`, we'll discuss several of them now.



## Setting arguments in `make_diagram`

In the previous vignette and the example above, you saw that it is possible to start customizing the diagram by providing specific settings when calling the `make_diagram` function. The help file for the function shows you what settings you can customize, you can look at it with this command
...
flows = list(S_flows = c("n", "-b*S*I", "-m*S"), 
             ...
             R_flows = c("g*I", "-m*R"))
sirmodel = list(varlabels = varlabels, flows = flows, varnames = varnames)
```
...
nice_diagram <- make_diagram(sir_diagram_list,  
...
simpler_diagram_1 <- make_diagram(sir_diagram_list,  
                             label_flows = FALSE,
                             external_flows = TRUE,
...
simpler_diagram_2 <- make_diagram(sir_diagram_list,  
                             label_flows = TRUE,
...
diag1 <- make_diagram(pp_diagram_list,  
                             node_outline_color = NA,
                             node_fill_color = c("#6aa4c8", "#eb5600", "#1a9988"),
                             node_text_color = "white",
                             node_text_size = 10,
...
diag2 <- make_diagram(pp_diagram_list,  
                             node_outline_color = NA,
                             node_fill_color = c("#6aa4c8", "#eb5600", "#1a9988"),
                             node_text_color = "white",
                             node_text_size = 10,
