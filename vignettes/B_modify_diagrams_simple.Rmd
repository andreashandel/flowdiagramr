...
Note that providing values for `var_text_size` and `flow_text_size` overwrites the values that were provided in `prepare_diagram` and stored in the output data frames (see `plot_label_size` column in the output data frames).
...
sirmodel = list(varlabels = varlabels, flows = flows)
sirsettings = list(varnames = varnames, varlocations = varlocations, use_varnames = TRUE)
```
...
diagram_settings = list(var_text_size = 4)
# make  diagram
...
diagram_settings$var_text_size <- 8
sir_diagram <- make_diagram(sir_diagram_list, diagram_settings)
plot(sir_diagram)
```
