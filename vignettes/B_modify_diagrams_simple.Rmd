...
#make diagram
my_diagram <- make_diagram(diagram_list = diagram_list)
#plot diagram
plot(my_diagram)
```
...
You always need to provide a `model_list` input to `prepare_diagram`. In addition, you can provide an optional `model_settings` argument, which allows you to influence the placement of the variables/boxes and arrows. Check the help file with `help('prepare_diagram')` for all the details. Here we explore some of the options through a few examples.


### Example 1

We will explore an extension of the SIR model. For this extension, we also include natural births and deaths. We assume new births only enter the **S** compartment at some fixed flow, **n**, while deaths occur at rate **m** out of all compartments. The model is specified as follows
...
We now specify an additional `model_settings` argument called `varlocations`. 
This argument is a matrix and should contain the model variables aligned in a grid of rows and columns. This will correspond to their placement in the diagram. 
In this example, we decided that instead of having all three variables on a single row (the default), we want **S** and **R** on a top row in the left and right corners, and **I** on a row below in the middle. 

To accomplish that, we define a grid/matrix with 2 rows and 3 columns and in that matrix, we place the **S** and **R** compartments in the left and right corner of the top row and the **I** compartment in the middle of the bottom row. All other slots are empty, and need to be specified as empty strings. The following code accomplishes that.
...
For this simple model, having all compartments along a single row, the default setting of **`r pkg`**, would be ok. But unless your model is very simple, you likely do not want all variables/boxes on the same row. Thus, specifying `varlocations` is most often very useful and necessary to produce a decent looking diagram.
...
```{r}
sirsettings = list(varlocations = matrix(data = c("S", "I", "R"), nrow = 3,  byrow = TRUE),
                   varbox_y_scaling = 0.5,
                   varspace_y_scaling = 0.5
                   )
```
...
**MODEL CHECK. CHANGING Y SCALING HERE ALSO SEEMS TO CHANGE BOX SIZE, LABELS DON'T FIT ANYMORE. CAN ONE MAKE IT SUCH THAT THERE IS NO RELATIVE ADJUSTMENT OF SOME COMPONENTS AS OTHERS CHANGE? SAME ISSUE HAPPENS NOT ONLY FOR FIGURE DISPLAY INSIDE VIGNETTE BUT ALSO WHEN SAVED WITH GGSAVE.**
...
model_settings = list(varbox_x_scaling = 2)
...
We also specify some optional settings for `make_diagram()` to influence the look of the diagram.

```{r, fig.height=5}
diagram_settings <- list(
       var_fill_color = c("#6aa4c8", "#eb5600", "#1a9988", "#2987c2", "#e38b59", "#5c948c", "#e8e656"),
       var_label_text = c("Susceptible Children","Infected Children","Recovered Children",
