...
Quick changes to the layout can be made by supplying `prepare_diagram()` with some additional settings, and by using the `update_diagam()` function. Let's look at both.
...
The placement of the flow labels could be improved, which is not hard to do with **`r pkg`** and you'll learn soon.
...
## Changing settings with `update_diagram()`
